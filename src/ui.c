...
int ui_set_theme(const char *theme_name) {
  if (str_compare(theme_name, "hacker") == 0) {
    current_theme.bg =
        BG_DEFAULT; /* Use default terminal bg to avoid messy boxes */
...
void ui_dashboard(void) {
  ...
  /* Box Width: 50 chars inside, 2 leading spaces = 52 total columns */
  const char *top    = "  ╔════════════════════════════════════════════════╗";
  const char *header = "  ║                 SYSTEM DASHBOARD               ║";
  const char *mid    = "  ╠════════════════════════════════════════════════╣";
  const char *bottom = "  ╚════════════════════════════════════════════════╝";

  screen_set_color(current_theme.fg_accent, current_theme.bg);
  screen_print_line(top);
  screen_print_line(header);
  screen_print_line(mid);
  ...
  char pct_buf[16];
  int_to_string(mem_percent, pct_buf);
  screen_print(" ");
  screen_print(pct_buf);
  screen_print("%");

  /* Calculation: 
   * "  ║  " (5) + "RAM Usage: " (11) + "[...]" (22) + " " (1) + pct + "%" (1) = 40 + len(pct)
   * Target is 51 before the closing ║. 
   * Padding = 51 - (40 + len(pct)) = 11 - len(pct).
   */
  int ram_len = 40 + str_length(pct_buf);
  for (int i = 0; i < (51 - ram_len); i++) screen_print(" ");
  screen_print_line("║");

  /* ── Processes ── */
  screen_print("  ║  Processes: ");
  char proc_buf[16];
  int_to_string(proc_count, proc_buf);
  screen_print(proc_buf);
  /* 5 (prefix) + 11 (label) + len(proc) = 16 + len(proc). 
   * Padding = 51 - (16 + len(proc)) = 35 - len(proc).
   */
  int proc_len = 16 + str_length(proc_buf);
  for (int i = 0; i < (51 - proc_len); i++) screen_print(" ");
  screen_print_line("║");

  /* ── Files ── */
  screen_print("  ║  Files:     ");
  char file_buf[16];
  int_to_string(file_count, file_buf);
  screen_print(file_buf);
  /* 5 (prefix) + 11 (label) + len(file) = 16 + len(file). 
   * Padding = 51 - (16 + len(file)) = 35 - len(file).
   */
  int file_len = 16 + str_length(file_buf);
  for (int i = 0; i < (51 - file_len); i++) screen_print(" ");
  screen_print_line("║");

  screen_set_color(current_theme.fg_accent, current_theme.bg);
  screen_print_line(bottom);
  screen_newline();
  ui_apply_theme();
}

void ui_draw_footer(int mem_used, int mem_total, int process_count,
                    const char *health) {
  screen_newline();
  screen_set_color(current_theme.bg,
                   current_theme.fg_primary); /* Invert colors for footer */
